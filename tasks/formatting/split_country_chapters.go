...
func main() {
	...
	// Match "_One_", "_Two_", ... "_Ten_" (chapter markers in this edition).
	chapterPattern := regexp.MustCompile(`(?i)^\s*_(One|Two|Three|Four|Five|Six|Seven|Eight|Nine|Ten)_\s*$`)
	...
	if len(chapterStarts) == 0 {
		fmt.Println("No chapter markers found in country.txt")
		return
	}
	...
	for i := 0; i < sectionCount; i++ {
		...
		filename := fmt.Sprintf("Country_Section_%d.txt", i+1)
