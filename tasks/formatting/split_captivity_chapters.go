...
func main() {
	...
	// Match standalone "ONE", "TWO", ... "TEN" (chapter markers in this edition).
	chapterPattern := regexp.MustCompile(`^\s*(ONE|TWO|THREE|FOUR|FIVE|SIX|SEVEN|EIGHT|NINE|TEN)\s*$`)
	...
	if len(chapterStarts) == 0 {
		fmt.Println("No chapter markers found in captivity.txt")
		return
	}
	...
	// Section 1 = front matter + chapter One.
	...
	for i := 0; i < sectionCount; i++ {
		...
		filename := fmt.Sprintf("Captivity_Section_%d.txt", i+1)
