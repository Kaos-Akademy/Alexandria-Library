...
func main() {
	...
	// Match "Chapter One", "Chapter Two", ... (word form, title case).
	chapterPattern := regexp.MustCompile(`(?i)^\s*Chapter [A-Za-z]+(-[A-Za-z]+)?\s*$`)
	...
	if len(chapterStarts) == 0 {
		fmt.Println("No chapter markers found in pastoral.txt")
		return
	}
	...
	for i := 0; i < sectionCount; i++ {
		...
		filename := fmt.Sprintf("Pastoral_Section_%d.txt", i+1)
