...
func main() {
	...
	// Match standalone "1", "2", ... "11" (chapter markers; 1–2 digits to exclude year).
	chapterPattern := regexp.MustCompile(`^\s*[0-9]{1,2}\s*$`)
	...
	if len(chapterStarts) == 0 {
		fmt.Println("No chapter markers found in town.txt")
		return
	}
	...
	for i := 0; i < sectionCount; i++ {
		...
		filename := fmt.Sprintf("Town_Section_%d.txt", i+1)
