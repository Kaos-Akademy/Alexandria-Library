...
func main() {
	...
	// Match "◆ 1 ◆", "◆ 2 ◆", ... (chapter markers in this edition).
	chapterPattern := regexp.MustCompile(`^\s*◆\s*[0-9]+\s*◆\s*$`)
	...
	if len(chapterStarts) == 0 {
		fmt.Println("No chapter markers found in pied.txt")
		return
	}
	...
	for i := 0; i < sectionCount; i++ {
		...
		filename := fmt.Sprintf("Pied_Section_%d.txt", i+1)
