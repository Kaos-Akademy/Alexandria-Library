...
func main() {
	...
	// Match "CHAPTER ONE", "CHAPTER TWO", ... "CHAPTER THIRTEEN" (word form).
	chapterPattern := regexp.MustCompile(`^\s*CHAPTER [A-Z]+\s*$`)
	...
	if len(chapterStarts) == 0 {
		fmt.Println("No chapter markers found in ruined.txt")
		return
	}
	...
	for i := 0; i < sectionCount; i++ {
		...
		filename := fmt.Sprintf("Ruined_Section_%d.txt", i+1)
