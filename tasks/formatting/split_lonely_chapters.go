...
func main() {
	...
	// Match "Chapter 1", "Chapter 2", ... (numeric).
	chapterPattern := regexp.MustCompile(`^\s*Chapter [0-9]+\s*$`)
	...
	if len(chapterStarts) == 0 {
		fmt.Println("No chapter markers found in lonely.txt")
		return
	}
	...
	for i := 0; i < sectionCount; i++ {
		...
		filename := fmt.Sprintf("Lonely_Section_%d.txt", i+1)
