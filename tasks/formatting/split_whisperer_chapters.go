...
func main() {
	...
	// Match standalone "1", "2", ... (chapter markers in this edition).
	chapterPattern := regexp.MustCompile(`^\s*[0-9]+\s*$`)
	...
	if len(chapterStarts) == 0 {
		fmt.Println("No chapter markers found in whisperer.txt")
		return
	}
	...
	for i := 0; i < sectionCount; i++ {
		...
		filename := fmt.Sprintf("Whisperer_Section_%d.txt", i+1)
