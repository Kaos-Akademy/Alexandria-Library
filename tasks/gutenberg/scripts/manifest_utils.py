...
def upload_genre() -> str:
    return os.environ.get("UPLOAD_GENRE", "philosophy")


def upload_target(default: int = 500) -> int:
    raw = os.environ.get("UPLOAD_TARGET", "").strip()
    if raw:
        return int(raw)
    return default
...
def run_state_path() -> str:
    return f"tasks/gutenberg/manifests/{upload_genre()}_500_run_state.json"


def prep_manifest_path() -> str:
    return f"tasks/gutenberg/manifests/{upload_genre()}_500_prep.json"


def master_manifest_path() -> str:
    return f"tasks/gutenberg/manifests/{upload_genre()}.json"


def prep_lock_path() -> str:
    return f"tasks/gutenberg/.{upload_genre()}_prep.lock"


def __getattr__(name: str):
    """Resolve genre-aware paths at access time so UPLOAD_GENRE env is always current."""
    if name == "RUN_STATE_PATH":
        return run_state_path()
    if name == "PREP_MANIFEST":
        return prep_manifest_path()
    if name == "MASTER_MANIFEST":
        return master_manifest_path()
    if name == "PREP_LOCK_PATH":
        return prep_lock_path()
    raise AttributeError(f"module {__name__!r} has no attribute {name!r}")


def batch_glob_pattern() -> str:
    prefix = os.environ.get("UPLOAD_BATCH_PREFIX", f"{upload_genre()}_next25")
        ...
        try:
            ...
            if holder != os.getpid():
                if pid_alive(holder):
                    ...
                    sys.exit(1)
                # Stale lock from a dead worker — remove so prep can resume.
                os.remove(path)
...
def seed_run_state() -> dict:
    ...
    batch_prefix = os.environ.get("UPLOAD_BATCH_PREFIX", f"{upload_genre()}_next25")
    ...
    master = load_json(master_manifest_path())
    deferred_ids = [
        e["gutenberg_id"]
