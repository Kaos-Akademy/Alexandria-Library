...
DRY_RUN=false
MAX_BATCHES=0
# Preserve CLI args for detached re-exec (arg parse consumes "$@").
SCRIPT_ARGS=("$@")
UPLOAD_GENRE="${UPLOAD_GENRE:-philosophy}"
...
LOG_DIR="logs/${UPLOAD_GENRE}_${UPLOAD_TARGET}"
mkdir -p "$LOG_DIR"
RUN_LOG="${UPLOAD_RUN_LOG:-${LOG_DIR}/run_$(date +%Y%m%d_%H%M%S).log}"

# When stdout is not a TTY (nohup, Cursor background tasks), re-exec in a new
# session so SIGHUP from the launching shell does not kill prep/upload mid-run.
if [[ "${UPLOAD_FOREGROUND:-}" != "1" && "${UPLOAD_DETACHED:-}" != "1" && ! -t 1 ]]; then
  export UPLOAD_DETACHED=1
  export UPLOAD_RUN_LOG="$RUN_LOG"
  nohup env \
    UPLOAD_GENRE="$UPLOAD_GENRE" \
    UPLOAD_DETACHED=1 \
    UPLOAD_RUN_LOG="$RUN_LOG" \
    UPLOAD_BATCH_PREFIX="${UPLOAD_BATCH_PREFIX}" \
    UPLOAD_BATCH_SIZE="${UPLOAD_BATCH_SIZE}" \
    UPLOAD_BOOK_CONCURRENCY="${UPLOAD_BOOK_CONCURRENCY}" \
    UPLOAD_KEY_COUNT="${UPLOAD_KEY_COUNT}" \
    UPLOAD_REPAIR_CONCURRENCY="${UPLOAD_REPAIR_CONCURRENCY}" \
    UPLOAD_LAUNCH_STAGGER_MS="${UPLOAD_LAUNCH_STAGGER_MS}" \
    UPLOAD_TARGET="${UPLOAD_TARGET}" \
    python3 -c "import os,sys; os.setsid(); os.execvpe(sys.argv[1], sys.argv[1:], os.environ)" \
    bash "$0" "${SCRIPT_ARGS[@]}" >>"$RUN_LOG" 2>&1 &
  echo "Detached upload run PID $! (setsid via python). Log: $RUN_LOG"
  exit 0
fi
...
echo "=== Upload run started $(date -u +%Y-%m-%dT%H:%M:%SZ) genre=${UPLOAD_GENRE} target=${UPLOAD_TARGET} detached=${UPLOAD_DETACHED:-0} ==="
echo "Log: $RUN_LOG"
...
if [[ "$SKIP_PREP" != true && "$PREP_DONE" != "True" ]]; then
  ...
  if [[ "$DRY_RUN" != true && "$PREP_LIMIT" -eq 0 ]]; then
    python3 -c "
import json
