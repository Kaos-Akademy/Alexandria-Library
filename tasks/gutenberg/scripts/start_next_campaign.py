...
def main() -> None:
    ...
    genre = os.environ.get("UPLOAD_GENRE", "philosophy")
    stamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    archive = rel(f"tasks/gutenberg/manifests/{genre}_500_run_state_campaign{campaign}_{stamp}.json")
    ...
    target = int(os.environ.get("UPLOAD_TARGET", state.get("target", 500)))
    new_state = {
        "target": target,
        "uploaded_count": 0,
        "prep_done": False,
    ...
    print(
        f"Campaign {new_campaign}: target={target} uploaded_count=0 prep_done=false "
