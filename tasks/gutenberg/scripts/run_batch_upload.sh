...
TARGET="${UPLOAD_TARGET:-500}"
LOG_DIR="logs/${GENRE}_${TARGET}"
BOOK_CONCURRENCY="${UPLOAD_BOOK_CONCURRENCY:-15}"
KEY_COUNT="${UPLOAD_KEY_COUNT:-25}"
REPAIR_CONCURRENCY="${UPLOAD_REPAIR_CONCURRENCY:-5}"
LAUNCH_STAGGER_MS="${UPLOAD_LAUNCH_STAGGER_MS:-10}"
MAX_BOOK_RETRIES="${UPLOAD_MAX_BOOK_RETRIES:-3}"
GUTENBERG_BIN="${GUTENBERG_BIN:-bin/gutenberg}"
SIGNER="${UPLOAD_SIGNER:-Prime-librarian}"
...
if [[ ! -x "$GUTENBERG_BIN" ]]; then
  echo "Building $GUTENBERG_BIN..."
  mkdir -p bin
  go build -o "$GUTENBERG_BIN" ./cmd/gutenberg
fi
...
UPLOAD_COMMON_FLAGS=(
  ...
  -max-book-retries "$MAX_BOOK_RETRIES"
)

# Initial upload pass
echo "Initial upload (concurrency ${BOOK_CONCURRENCY}, keys ${KEY_COUNT}, stagger ${LAUNCH_STAGGER_MS}ms, book-retries ${MAX_BOOK_RETRIES})..."
set +e
"$GUTENBERG_BIN" upload \
  "${UPLOAD_COMMON_FLAGS[@]}" \
  -book-concurrency "$BOOK_CONCURRENCY" 2>&1 | tee "$UPLOAD_LOG"
UPLOAD_RC=${PIPESTATUS[0]}
set -e
echo "Initial upload exit code: $UPLOAD_RC"
if [[ "$UPLOAD_RC" -eq 2 ]]; then
  echo "ERROR: storage wall — Prime-librarian account cannot pass Flow storage fee checks. Uploads paused until resolved." >&2
  exit 2
fi

# Auto-repair pass if any split entries remain (should be rare after inline retries)
